...
public class LimitExpression 
{
	...
	private static final double mutationRate = 0.5;//should be .015
	private static final double expansionRate = .5; //under normal parameters, how likely is a gene to expand?
	private static final double regressionRate = .75;//under normal parameters, how likely is a gene to regress?
				...
				else if(mutationType == 2)//substitution
				{
					...
					if(current.getExpType().equalsIgnoreCase("Number") || current.getExpType().equalsIgnoreCase("Variable"))
					{
						...
						if(numOrVar == 0)
						{
							...
							case 0: ((UnaryOperator)current.getPreviousOperator()).setExp(new Number(gen.nextInt(1000)));break;
							case 1: ((BinaryOperator)current.getPreviousOperator()).setExp1(new Number(gen.nextInt(1000)));break;
							case 2: ((BinaryOperator)current.getPreviousOperator()).setExp2(new Number(gen.nextInt(1000)));break;
